...
function init(){
  ...
  _preCheck(function(err){
    ...
    execSyncInherit(`useradd ${username} --system -m --base-dir=${path.dirname(homeDir)} --skel=skel --shell=/bin/false`, {cwd: path.join(__dirname, '../')});
    console.log('user add success.');
    execSyncInherit(`chmod 755 ${homeDir}`); // Some system default mask is 700.
